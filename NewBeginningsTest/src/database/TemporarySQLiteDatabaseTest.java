...
/**
 * This is a temporary test to give me an entry point to make sure the database
 * stuff is working with file ops
 */
public class TemporarySQLiteDatabaseTest {
	...
	public void setUp() throws Exception {
		...
		this._wrapper = new SQLiteWrapper(testDatabasePath);
		this._animalDB = new AnimalDatabaseSQLite(this._wrapper);
		...
		this._fakeCats.add(new Cat("NB-14-9", "Garfield", Calendar
				.getInstance(), "M", "Calico", "Dark Orange", true, Calendar
	...
	public void test() {
		for (Cat cat : this._fakeCats) {
			this._animalDB.addNewCat(cat);
		}

		String filter = "G";
		System.out.println("Filtered cats with filter " + filter + ":");
		List<Cat> filteredCats = this._animalDB.getFilteredCats(
				SearchFilterType.Name, filter);
		...
		Cat idCat = this._animalDB.getSingleCat("NB-14-2");
		...
		Cat newIDCat = new Cat("NB-14-2", "Peppermint", Calendar.getInstance(),
				"M", "American Curl", "Black with White Stripes", true,
				Calendar.getInstance(), Calendar.getInstance());
		this._animalDB.updateCat(newIDCat);
		System.out.println("Getting cat with ID NB-14-2:");
		Cat retrievedNewIdCat = this._animalDB.getSingleCat("NB-14-2");
		...
		Cat dateCat = new Cat("NB-14-789", "Smeagol", birthdate, "M",
				"Siamese", "White", false, Calendar.getInstance(),
				Calendar.getInstance());
		System.out.print("\n\nDisplay age: ");
		...
		System.out.print("\\");
		System.out.print(dateCat.getBirthdate().getDisplayName(Calendar.MONTH,
				Calendar.SHORT, Locale.US));
		System.out.print("\\");
		System.out.println(dateCat.getBirthdate().get(Calendar.DAY_OF_MONTH));

		this._animalDB.addNewCat(dateCat);

		Cat retrievedDateCat = this._animalDB.getSingleCat("NB-14-789");
		System.out.print("Birthdate after storage:  ");
		System.out.print(retrievedDateCat.getBirthdate().get(Calendar.YEAR));
		System.out.print("\\");
		System.out.print(retrievedDateCat.getBirthdate().getDisplayName(
				Calendar.MONTH, Calendar.SHORT, Locale.US));
		System.out.print("\\");
		System.out.println(retrievedDateCat.getBirthdate().get(
